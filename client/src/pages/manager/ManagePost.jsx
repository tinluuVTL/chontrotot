  ...
  return (
    <div className="w-full h-full">
      <div className="flex justify-between py-4 lg:border-b px-4 items-center">
        ...
        {current.rroles.some(
          (el) => el.roleCode === "ADMIN" || el.roleCode === "MANAGER"
        ) ? (
          <Link
          ...
          <span className="py-2 px-4 bg-blue-600 text-white flex justify-center items-center rounded-md opacity-50 cursor-not-allowed">
            Tạo mới
          </span>
        )}
      </div>
          ...
          <table className="max-w-full w-full overflow-x-auto">
            <thead>
              <tr>
                ...
                <th className="border p-3 text-center">Ngày Cập Nhập</th>
                ...
                <tr key={el.id}>
                  ...
                  <td className="border p-3 text-center">
                    <span className="flex items-center gap-4 justify-center">
                      <span
                        onClick={
                          current.rroles.some((el) => el.roleCode === "ADMIN" || el.roleCode === "MANAGER")
                            ? () =>
                                navigate(
                                  `/${pathname.manager.LAYOUT}/${pathname.manager.UPDATE_POST}/${el.id}`
                                )
                            : undefined
                        }
                      ...
                      <span
                        onClick={() => handleRemove(el.id)} // Cho phép cả ADMIN và MANAGER xóa
                        className={`cursor-pointer hover:text-blue-600 ${
                          current.rroles.some((el) => el.roleCode === "ADMIN" || el.roleCode === "MANAGER")
                            ? ""
                            : "opacity-50 cursor-not-allowed"
